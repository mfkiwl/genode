...
class Subsystem
{
	private:
		...
		Genode::Signal_context_capability _next_sigh;
		...
		void _handle_import(unsigned)
		{
			...
			/* trigger next step */
			Genode::Signal_transmitter(_next_sigh).submit();
		}
	...
	public:
		...
		Subsystem(Server::Entrypoint &ep, char const *name,
		          Genode::Signal_context_capability next_sigh)
		:
			_ep(ep),
			_name(name),
			_next_sigh(next_sigh),
...
struct Server::Main
{
	...
	void _handle_step(unsigned cnt)
	{
	...
	Genode::Signal_rpc_member<Main> _step_dispatcher =
		{ _ep, *this, &Main::_handle_step };

	Subsystem _admin { _ep, "noux",  _step_dispatcher };
	Subsystem _hobby { _ep, "linux", _step_dispatcher };
	Subsystem _work  { _ep, "win7",  _step_dispatcher };
